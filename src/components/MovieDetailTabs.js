...
const WHITE = "#fff";
const GREY = "#707070";
const IN_THEATER_POSITION = 18;
const COMING_SOON_POSITION = 129;
const FONT_BOLD = 600;
const FONT_NORMAL = 500;

export default class Index extends Component {
  _animatedX = new Animated.Value(18);

  // Handle tab bar onclick
  tabBarOnClick = isShowtime => {
    if (isShowtime != this.props.isShowtime) {
      this.props.handleTabIndexChange(); // Change tab status in the parent component
      this.animateIndicator(isShowtime);
    }
  };
  // Animate indicator when tab bar onclick happens
  animateIndicator = isShowtime => {
    Animated.timing(this._animatedX, {
      toValue: isShowtime ? IN_THEATER_POSITION : COMING_SOON_POSITION
    }).start();
  };
